...
import { getMemoryFileCandidates } from './runtime_paths';
...
function isLobsterMemorySelection(input: string): boolean {
...
async function loadLobsterMemoryWorldRequirement(): Promise<string> {
  for (const filePath of getMemoryFileCandidates()) {
    try {
...
async function completePendingSetup(
  ...
  if (pendingSetup.stage === "awaiting_world_requirement") {
    ...
    return {
      content: buildNarrativeStylePrompt(
        pendingSetup.slotId,
        pendingSetup.playerName,
        isLobsterMemorySelection(normalized.replace(/^世界观\s+/, "").trim()) ? "已读取并应用龙虾的 Memory 文件" : worldRequirement
      ),
      slotId: pendingSetup.slotId,
      shouldContinue: true
    };
...
function parseGameCommand(message: string): { kind: string; slotId?: string } | null {
  ...
  if (newGameMatch) {
    ...
    if (!slotNumber) {
      if (normalized.includes("第二")) slotNumber = 2;
      else if (normalized.includes("第三")) slotNumber = 3;
      else if (normalized.includes("第四")) slotNumber = 4;
      else if (normalized.includes("第五")) slotNumber = 5;
      else slotNumber = 1;
    }
