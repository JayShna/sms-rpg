...
exports.processGameInput = processGameInput;
exports.handleOpenClawMessage = handleOpenClawMessage;
const kimi_client_1 = require("./kimi_client");
const game_engine_1 = require("./game_engine");
const dialog_store_1 = require("./dialog_store");
...
const LOBSTER_MEMORY_OPTION = "4";
let promptCache = null;
function toSlotId(slotNumber) {
    return `save_${String(slotNumber).padStart(3, "0")}`;
}
function fromSlotId(slotId) {
...
function buildApiPrompt(slotId) {
    return [
...
function buildStartPrompt(slotId, existingSave) {
...
function buildWorldRequirementPrompt(slotId, playerName) {
    return [
        `主角已设为：${playerName}`,
        `接下来请回复你想要的世界观，用于创建存档 ${fromSlotId(slotId)} 的新游戏。`,
        "你可以直接输入世界观描述，也可以参考下面的固定选项：",
        "1. 武侠修仙，江湖门派与朝廷暗斗",
        "2. 朝堂权谋，边军崛起，乱世争龙",
        "3. 世界观 赛博修仙末日废土",
        "4. 读取龙虾的 memory.md"
...
function isLobsterMemorySelection(input) {
    const normalized = input.trim();
    return normalized === LOBSTER_MEMORY_OPTION
        || normalized === "读取龙虾的 memory.md"
        || normalized === "读取龙虾的memory.md";
}
async function loadLobsterMemoryWorldRequirement() {
    for (const filePath of (0, runtime_paths_1.getMemoryFileCandidates)()) {
        try {
            ...
            return `【龙虾的 Memory 设定文档】\n${trimmed.slice(0, 4000)}\n【以上为专属设定，请基于这些信息生成世界】`;
        }
        catch {
            continue;
        }
    }
    throw new Error("未找到可用的 MEMORY.md 或 memory.md。请确认 OpenClaw 工作区内存在该文件，或直接输入世界观。");
}
function buildNarrativeStylePrompt(slotId, playerName, worldRequirement) {
    return [
        `主角：${playerName}`,
...
function buildNewGameContent(storyOverview, openingNarrative, runtimeOverview, slotId) {
    return [
...
function buildContinueContent(slotId, runtimeOverview) {
    return [
...
async function loadPrompts() {
    if (!promptCache) {
        const [genesisPrompt, canonPrompt, episodePrompt] = await Promise.all([
            (0, kimi_client_1.loadGenesisPrompt)(),
            (0, kimi_client_1.loadPromptFile)('prompts/canon_guardian_prompt.md'),
            (0, kimi_client_1.loadPromptFile)('prompts/episode_summary_prompt.md')
        ]);
        promptCache = { genesisPrompt, canonPrompt, episodePrompt };
    }
    return promptCache;
}
function extractApiKey(message) {
...
function normalizeNarrativeStyle(input) {
...
function isValidNarrativeStyleInput(input) {
...
function buildUpgradePromptText() {
    const purchaseUrl = (0, license_service_1.getPurchaseUrl)();
    return [
        "如果你想提升存档槽数量，可以解锁完整版。",
        purchaseUrl ? `爱发电链接：${purchaseUrl}` : "请在环境变量 SMS_PRODUCT_URL 中配置你的爱发电链接。",
        "",
        (0, license_service_1.formatUnlockInstructions)()
    ].join("\n");
}
function buildChapterPromotionText(completedTurn) {
...
function buildPendingStageRetryHint(pendingSetup) {
...
async function ensureSlotAllowed(slotId) {
    const status = await (0, license_service_1.getLicenseStatus)();
    const allowed = (0, license_service_1.getAccessibleSaveSlotCount)(status);
...
async function listSlotStatus(userId) {
    const status = await (0, license_service_1.getLicenseStatus)();
    const allowed = (0, license_service_1.getAccessibleSaveSlotCount)(status);
    const lines = [(0, game_engine_1.buildLicenseBannerText)(status), "", "【存档列表】"];
    for (let slotNumber = 1; slotNumber <= TOTAL_SAVE_SLOTS; slotNumber += 1) {
        const slotId = toSlotId(slotNumber);
        const save = await (0, dialog_store_1.loadDialogSave)(userId, slotId);
...
async function handleUnlockMessage(message) {
    const normalized = normalizeMessage(message);
    if (normalized === "解锁") {
        return {
            content: (0, license_service_1.formatUnlockInstructions)(),
            slotId: "save_001",
            shouldContinue: true
        };
    }
    const code = normalized.replace(/^解锁\s+/, "").trim();
    const identity = (0, device_identity_1.getMachineIdentity)();
    const result = await (0, license_service_1.activateLicenseCode)(code, identity.machineId);
    return {
        content: [
            `[授权] ${result.message}`,
            result.ok ? `[授权] 机器摘要：${identity.summary}` : "",
            (0, license_service_1.formatLicenseStatus)(result.status)
        ].filter(Boolean).join("\n"),
        slotId: "save_001",
        shouldContinue: true
    };
}
async function continueGame(userId, slotId) {
    ...
    const runtime = (0, game_engine_1.createRuntimeFromSave)(slotId, save, status);
    return {
        content: buildContinueContent(slotId, (0, game_engine_1.buildRuntimeOverviewText)(runtime)),
        slotId,
        shouldContinue: true
    };
}
async function completePendingSetup(userId, input, pendingSetup, currentSlotId) {
    const normalized = normalizeMessage(input);
    if (normalized === "取消") {
        await (0, dialog_store_1.saveDialogSessionMeta)({
            userId,
            currentSlotId,
            pendingSetup: null
        });
    ...
    if (pendingSetup.stage === "awaiting_api_key") {
        ...
        try {
            const message = await (0, kimi_client_1.configureKimiApiKey)(apiKey);
            const guard = await ensureSlotAllowed(pendingSetup.slotId);
            if (!guard.ok) {
                await (0, dialog_store_1.saveDialogSessionMeta)({
                    userId,
                ...
                return {
                    content: [message, "", guard.message || "该存档槽当前不可用。", "", buildUpgradePromptText()].join("\n"),
                    slotId: pendingSetup.slotId,
                    shouldContinue: true
                };
            }
            ...
            return {
                content: [
                    message,
                    "",
                    buildStartPrompt(pendingSetup.slotId, (await (0, dialog_store_1.loadDialogSave)(userId, pendingSetup.slotId)) !== null)
                ].join("\n"),
                slotId: pendingSetup.slotId,
                shouldContinue: true
            };
    ...
    let worldRequirement = normalized.replace(/^世界观\s+/, "").trim();
    if (pendingSetup.stage === "awaiting_world_requirement") {
        if (!worldRequirement) {
            return {
        ...
        if (isLobsterMemorySelection(worldRequirement)) {
            try {
                worldRequirement = await loadLobsterMemoryWorldRequirement();
            }
            catch (error) {
                return {
                    content: `${formatDialogError(error)}\n请先准备好 OpenClaw 工作区中的 MEMORY.md / memory.md，或直接输入世界观内容。`,
                    slotId: pendingSetup.slotId,
                    shouldContinue: true
                };
            }
        ...
        return {
            content: buildNarrativeStylePrompt(pendingSetup.slotId, pendingSetup.playerName, isLobsterMemorySelection(normalized.replace(/^世界观\s+/, "").trim()) ? "已读取并应用龙虾的 Memory 文件" : worldRequirement),
            slotId: pendingSetup.slotId,
            shouldContinue: true
        };
    }
    try {
        ...
        const [client, licenseStatus] = await Promise.all([
            (0, kimi_client_1.getKimiClientConfigNonInteractive)(),
            (0, license_service_1.getLicenseStatus)()
        ]);
        const result = await (0, game_engine_1.createNewGameRuntime)(client, pendingSetup.slotId, pendingSetup.playerName, pendingSetup.worldRequirement, normalizeNarrativeStyle(normalized), licenseStatus);
        await (0, dialog_store_1.saveDialogSave)(userId, pendingSetup.slotId, (0, game_engine_1.toGameSave)(result.runtime));
        await (0, dialog_store_1.saveDialogSessionMeta)({
            userId,
            currentSlotId: pendingSetup.slotId,
            pendingSetup: null
        ...
        return {
            content: buildNewGameContent(result.storyOverview, result.openingNarrative, (0, game_engine_1.buildRuntimeOverviewText)(result.runtime), pendingSetup.slotId),
            slotId: pendingSetup.slotId,
            shouldContinue: true
        };
    }
...
function parseGameCommand(message) {
    const normalized = normalizeMessage(message);
    if (!normalized) {
        return null;
    }
    ...
    const newGameMatch = normalized.match(/^(?:开始新游戏|新游戏|创建新存档|创建新游戏|创建存档|创建第二存档|创建第三存档|创建第四存档|创建第五存档|创建第(\d+)存档)(?:\s+(\d+))?$/);
    if (newGameMatch) {
        let slotNumber = parseSlotNumber(newGameMatch[2]) || parseSlotNumber(newGameMatch[1]);
        if (!slotNumber) {
            if (normalized.includes("第二"))
                slotNumber = 2;
            else if (normalized.includes("第三"))
                slotNumber = 3;
            else if (normalized.includes("第四"))
                slotNumber = 4;
            else if (normalized.includes("第五"))
                slotNumber = 5;
            else
                slotNumber = 1;
        }
...
async function processGameInput(userId, input, preferredSlotId) {
    const message = normalizeMessage(input);
    const meta = await (0, dialog_store_1.loadDialogSessionMeta)(userId);
    ...
    if (directApiKey && !meta.pendingSetup) {
        try {
            const resultMessage = await (0, kimi_client_1.configureKimiApiKey)(directApiKey);
            return {
                content: [
                    resultMessage,
                    "",
    ...
    if (meta.pendingSetup) {
        ...
        if (command?.kind === "license_status") {
            return {
                content: (0, license_service_1.formatLicenseStatus)(await (0, license_service_1.getLicenseStatus)()),
                slotId: activeSlotId,
                shouldContinue: true
            };
        }
    ...
    if (command?.kind === "start_new_game" && command.slotId) {
        if (!await (0, kimi_client_1.hasConfiguredKimiApiKey)() || !await (0, kimi_client_1.isConfiguredKimiApiKeyUsable)()) {
            await (0, dialog_store_1.saveDialogSessionMeta)({
                userId,
        ...
        const existingSave = await (0, dialog_store_1.loadDialogSave)(userId, command.slotId);
        await (0, dialog_store_1.saveDialogSessionMeta)({
            userId,
            currentSlotId: command.slotId,
            pendingSetup: {
    ...
    const save = await (0, dialog_store_1.loadDialogSave)(userId, activeSlotId);
    if (!save) {
        return {
            content: [
                "当前还没有可继续的游戏。",
    ...
    const licenseStatus = await (0, license_service_1.getLicenseStatus)();
    const runtime = (0, game_engine_1.createRuntimeFromSave)(activeSlotId, save, licenseStatus);
    const resolvedAction = (0, game_engine_1.resolvePlayerActionInput)(message, runtime.currentOptions);
    ...
    try {
        ...
        const turnResult = await (0, game_engine_1.executeTurn)(runtime, resolvedAction.playerInput, resolvedAction.selectedOption, client, prompts);
        await (0, dialog_store_1.saveDialogSave)(userId, activeSlotId, (0, game_engine_1.toGameSave)(turnResult.runtime));
        await (0, dialog_store_1.saveDialogSessionMeta)({
            userId,
...
async function handleOpenClawMessage(message, context) {
